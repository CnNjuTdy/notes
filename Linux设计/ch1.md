...
有一些情况下，我们可能需要把命令输出的结果存在文件中，这时可以使用重定向完成。

`cmd > a.txt`可以把cmd输出的结果**重定向**到文件中。

有些命令本身可以指定参数使得结果输出到文件中，这样的机制和重定向机制是不一样的。

实际上，命令结果输出屏幕上的机制是系统调用标准输入，标准输出，标准错误接口，而重定向机制是不调用这些接口，而是输出到文件中。
...
//标准输出重定向到killout.txt
//标准错误重定向到kukkerr.txt
kill -HUP 1234 &> kill.txt
//标准输出和错误都重定向kill.txt
```

### 6.2 管道

一个进程的标准输出直接重定向为另一个进程的标准输入

`ls|wc -l`:ls的结果将会变成wc -l的输入
...
(set) 变量名 = 变量值  export 变量名

### 6.4 三大神器

#### 6.4.1 find命令

find可以用很复杂的参数找到想要的文件
...
```java
sed 's/      要找的东西（正则表达式）    /  要替换的东西（正则表达式）    /g' code1.cpp
```
